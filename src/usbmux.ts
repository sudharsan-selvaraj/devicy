...
function createListener(): net.Socket {
  const conn = net.connect(address as any);
 ...
 * - rejects  {Error}
 */
function connectToDevice(deviceID: string, devicePort: number): Promise<net.Socket> {
 ...
 * - rejects  {Error}
 */
function findDevice(opts?: { timeout?: number; udid?: string }): Promise<number> {
  return new Promise(function (resolve, reject) {
